...
class Upload {
    ...
    private function getSaveName($file) {
        ...
        if (empty($rule)) { //保持文件名不变
            ...
            if (($rule=='md5' || $rule=='sha1') && $this->hash==true ) {
                $savename = $file[$rule];

                //$f4name = md5_file($filename['tmp_name']);
                $savename = substr($savename,0,12).'-'.substr($savename,12,4).'-'.substr($savename,16,4).'-'.substr($savename,20,12).".".$savename['extension'];
